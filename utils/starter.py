...
async def start(thread: int, session_name: str, phone_number: str, proxy: [str, None]):
    ...
    while True:
        try:
            ...
            available_games = [game for game, is_available in config.GAMES.items() if is_available]
            if available_games:
                selected_game = random.choice(available_games)

                if selected_game == "512":
                    await clay.play_512(game_tries)
                elif selected_game == "stack":
                    await clay.play_stack(game_tries)

            if clay.iso_to_unix_time(start_time)+21600 > clay.current_time(start_time):
                sleep = clay.iso_to_unix_time(start_time)+21600 - clay.current_time(start_time)
