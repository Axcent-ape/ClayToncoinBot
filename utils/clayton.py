...
class ClayTon:
    ...
    async def tasks(self):
        ...
        task_bird = await (await self.session.post('https://tonclayton.fun/api/user/task-bird')).json()
        if not task_bird.get('claimed'):
            resp = await self.session.post("https://tonclayton.fun/api/user/task-bird-claim")

            if resp.status == 200:
                logger.success(f"Thread {self.thread} | {self.account} | Completed task «Play TonBird»! Reward: 200")
        ...
        task_diamore = await (await self.session.post('https://tonclayton.fun/api/user/task-diamore')).json()
        if not task_diamore.get('claimed'):
            resp = await self.session.post("https://tonclayton.fun/api/user/task-diamore-claim")

            if resp.status == 200:
                logger.success(f"Thread {self.thread} | {self.account} | Completed task «Play Diamore»! Reward: 200")
    ...
    async def play_512(self, game_tries: int, balance: int):
        while game_tries:
            ...
            if status:
                ...
                for i in range(8):
                    tile *= 2
                    await self.session.post('https://tonclayton.fun/api/game/save-max-tile', json={"maxTile": tile})
